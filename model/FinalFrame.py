...
import torch
import numpy as np
from model.Transformer import Transformer, GeometricModule
...
class GeometricConcat(nn.Module):
    def __init__(self, d_model = 128, d_coordinate = 2,d_state = 2 , d_tgt = 4):
        super(GeometricConcat, self).__init__()

        self.geom_module = GeometricModule(d_src =d_coordinate, d_model= d_model )
    ...
    def forward(self, coordinates , length, states, distances):
        geom, _  = self.geom_module(coordinates, length, distances)  # 1 * d_model
        state = self.states_mlp(states)  # n * d_model
        geom_expanded = geom.expand(state.shape[0] , -1)
        fusion_features = torch.cat((geom_expanded, state), dim = 1)
        result = self.fusion_mlp(fusion_features)
